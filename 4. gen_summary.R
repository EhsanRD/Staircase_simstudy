...
summary_res_fit <- function(nsim, S, K, m, ICC, CAC, theta,typ){
  ...
  pow_HH   <- sum(abs(res_est_fit$est_trt_HH)/res_est_fit$se_trt_HH > 1.96, na.rm=TRUE)/nsim_HH
  ...
  madj_ddf_KR_HH  <- mean(res_est_fit$adj_ddf_KR_HH, na.rm=TRUE)
  madj_ddf_KR_NSing_HH  <- mean(res_est_fit$adj_ddf_KR_NSing_HH, na.rm=TRUE)
  madj_ddf_Sat_HH  <- mean(res_est_fit$adj_ddf_Sat_HH, na.rm=TRUE)
  ...
  pow_KR_HH  <- sum(abs(res_est_fit$est_trt_HH)/res_est_fit$adj_se_KR_HH > tstat_KR_HH, na.rm=TRUE)/nsim_KR_HH
  ...
  p_cov_KR_HH <- ifelse(is.na(num_KR_cov_HH/nsim_KR_HH), NA, num_KR_cov_HH/nsim_KR_HH)
  ...
  pow_KR_NSing_HH  <- sum(abs(res_est_fit$est_trt_HH)/res_est_fit$adj_se_KR_NSing_HH > tstat_KR_NSing_HH, na.rm=TRUE)/nsim_KR_NSing_HH
  ...
  pow_Sat_HH <- sum(abs(res_est_fit$est_trt_HH)/res_est_fit$adj_se_Sat_HH > tstat_Sat_HH,na.rm=TRUE)/nsim_Sat_HH
  ...
  pow_BE   <- sum(abs(res_est_fit$est_trt_BE)/res_est_fit$se_trt_BE > 1.96, na.rm=TRUE)/nsim_BE
  ...
  madj_ddf_KR_BE <- mean(res_est_fit$adj_ddf_KR_BE, na.rm=TRUE)
  madj_ddf_KR_NSing_BE  <- mean(res_est_fit$adj_ddf_KR_NSing_BE, na.rm=TRUE)
  madj_ddf_Sat_BE <- mean(res_est_fit$adj_ddf_Sat_BE, na.rm=TRUE)
  ...
  pow_KR_BE <- sum(abs(res_est_fit$est_trt_BE)/res_est_fit$adj_se_KR_BE> tstat_KR_BE, na.rm=TRUE)/nsim_KR_BE
  ...
  p_cov_KR_BE <- ifelse(is.na(num_KR_cov_BE/nsim_KR_BE), NA, num_KR_cov_BE/nsim_KR_BE)
  ...
  pow_KR_NSing_BE <- sum(abs(res_est_fit$est_trt_BE)/res_est_fit$adj_se_KR_NSing_BE> tstat_KR_NSing_BE, na.rm=TRUE)/nsim_KR_NSing_BE 
  ...
  pow_Sat_BE <- sum(abs(res_est_fit$est_trt_BE)/res_est_fit$adj_se_Sat_BE > tstat_Sat_BE,na.rm=TRUE)/nsim_Sat_BE
  ...
  summary_emp_vals <- data.frame(nsim, S, K, m, ICC, CAC, theta,type=typ,power,
                                 nsim_HH=nsim_HH, 
                                 mesttrt_HH=mest_trt_HH, varesttrt_HH=varest_trt_HH,
                                 mestICC_HH=mest_ICC_HH,varICC_HH=varICC_HH, 
                                 madjddfKR_HH=madj_ddf_KR_HH,madjddfKRNSing_HH=madj_ddf_KR_NSing_HH,
                                 madjddfSat_HH=madj_ddf_Sat_HH,
                                 nsim_BE=nsim_BE, 
                                 mesttrt_BE=mest_trt_BE,varesttrt_BE=varest_trt_BE,
                                 mestICC_BE=mest_ICC_BE,varICC_BE=varICC_BE,
                                 mestCAC_BE=mest_CAC_BE,varCAC_BE=var_CAC_BE,
                                 madjddfKR_BE=madj_ddf_KR_BE,madjddfKRNSing_BE=madj_ddf_KR_NSing_BE,
                                 madjddfSat_BE=madj_ddf_Sat_BE,
                                 sIsSing_HH=s_IsSing_HH,pow_HH=pow_HH,
                                 powKR_HH=pow_KR_HH, powKRnSing_HH=pow_KR_NSing_HH,
                                 powSat_HH=pow_Sat_HH,
                                 pCov_HH=p_cov_HH,pCovKR_HH=p_cov_KR_HH,
                                 pCovKRnSing_HH=p_cov_KR_NSing_HH,pCovSat_HH=p_cov_Sat_HH,
                                 sIsSing_BE=s_IsSing_BE,pow_BE=pow_BE,
                                 powKR_BE=pow_KR_BE,powKRnSing_BE=pow_KR_NSing_BE,
                                 powSat_BE=pow_Sat_BE,
                                 pCov_BE=p_cov_BE,pCovKR_BE=p_cov_KR_BE,
                                 pCovKRnSing_BE=p_cov_KR_NSing_BE,pCovSat_BE=p_cov_Sat_BE,
  ...
  summary_emp_vals  <- summary_emp_vals %>%
    mutate(Biastrt_HH = mesttrt_HH - theta, Biastrt_BE = mesttrt_BE - theta,
           ...
           MCSE_HH=sqrt(varesttrt_HH/nsim_HH), MCSE_BE=sqrt(varesttrt_BE/nsim_BE))
...
# clusterExport(cl, c("file_name" ,"summary_res_fit","res_est_fit","pow" ,"VarSCcat","VarSClin"))  # Export necessary variables and functions to cluster
# clusterEvalQ(cl, {
#   library(tidyr)
#   library(parameters)
#   library(lmerTest)
